...
if [ -f elf/zImage ] && [ -f elf/kernel.elf ]; then
	...
	sed -i "11s/\".*\.elf\"/\"$kernel_name.elf\"/" $FINAL_PATH/zip_content/META-INF/com/google/android/updater-script; # looks for a xxxxx.img at line 11 and replaces it with the new name
